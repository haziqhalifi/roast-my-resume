...
export class RedisUsageStore {
  ...
  async consume(deviceId, ip, style) {
    ...
    try {
      ...
      await Promise.all([
        ...
        this.redis.incr("usage:lifetime:roasts"),
  ...
  async refund(device, ipHash, style, day) {
    ...
    try {
      await Promise.all([
        ...
        this.redis.decr("usage:lifetime:roasts"),
  ...
  async stats() {
    ...
    const [roasts, uniqueDevices, uniqueIps, styles, tokens, lifetimeRoasts] = await Promise.all([
      ...
      this.redis.get("usage:lifetime:roasts"),
    ]);
