...
export class UsageStore {
  ...
  constructor({
    ...
    this.salt = salt;
    this.now = now;
    this.data = { current: emptyDay(dayKey(this.now())), history: [], lifetime: { roasts: 0 } };
    this.load();
  }
  ...
  load() {
    ...
    try {
      ...
      if (parsed?.current?.day) this.data = parsed;
    } catch {
  ...
  check(deviceId, ip) {
    ...
    if (cur.roasts >= this.limits.globalPerDay) {
      return { allowed: false, reason: "global", remaining: 0, resetAt };
    }
    if (used >= this.limits.perDevicePerDay) {
      return { allowed: false, reason: "device", remaining: 0, resetAt };
    }
    if ((cur.ips[ipHash] || 0) >= this.limits.perIpPerDay) {
      return { allowed: false, reason: "ip", remaining: 0, resetAt };
    }
    return { allowed: true, remaining: this.limits.perDevicePerDay - used, resetAt, device, ipHash };
  }
